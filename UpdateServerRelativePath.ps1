...
function Update-ServerRelativePath ($SiteURL, $ListName) {
   ...
   #   $item= Get-PnPListItem -List $ListName -PageSize 1000 |Where-Object {(($_.FieldValues.FileLeafRef -contains "*Records_Template*") -or($_.FieldValues.FileLeafRef -contains "*Records Template*")-or($_.FieldValues.FileLeafRef -contains "*RecordsTemplate*")-or($_.FieldValues.FileLeafRef -contains "*Records-Template*")) }
    $item= Get-PnPListItem -List $ListName -PageSize 1000 
        # $item= Get-PnPListItem -List $ListName -PageSize 1000 -Query "<View><Query><Where><And><Contains><FieldRef Name='DocIcon' /><Value Type='Computed'>txt</Value></Contains><Or><Contains><FieldRef Name='FileLeafRef' /><Value Type='File'>Records Template</Value></Contains><Or><Contains><FieldRef Name='FileLeafRef' /><Value Type='File'>Records_Template</Value></Contains> <Contains><FieldRef Name='FileLeafRef' /><Value Type='File'>RecordsTemplate</Value> </Contains> </Or></Or></And></Where>
#</Query></View>"
  ...
  Write-Host -ForegroundColor Yellow "ItemCount" $item.Count
        ...
        for ($i = 0; $i -lt $item.Count ; $i++) {
           ...
           # Working  if(($item[$i].FieldValues.FileRef -notlike "*/Restricted*" ) -and ($item[$i].FieldValues.FileRef -like "*.txt" ))
